...
public class JMSCompania implements MessageListener, ExceptionListener
{
	 ...
	 */
	public final static String GET_ALL_NOTA_DEBITO_ASK = "GETALLNOTADEBITO";

	/**
	 * Atributo que representa, dentro del mensaje, la respuesta del requerimiento dar todos los videos.
	 */
	public final static String GET_ALL_NOTA_DEBITO_REPLY = "GETALLNOTADEBITORES";
	 ...
	 */
	public void setUpMaster(FestivAndes festivAndesMaster){
	 ...
	 */
	public static JMSCompania darInstacia(FestivAndes festivAndesMaster){
		instancia = instancia == null? new JMSCompania() : instancia;
	...
	public void setUpJMSManager(int numerApps, String myQueue, String topicAllFunciones){
		try {
			...
			setupSubscriptions();
			waiting = false;
			List<NotaDebito> notaDebito = new ArrayList<NotaDebito>();
			respuesta = new ListaNotaDebito(notaDebito);
	 ...
	 */
	public List<NotaDebito> getCompaniasResponse(String id) throws IncompleteReplyException, JMSException, NamingException, InterruptedException, NonReplyException {
		sendMessage(id); // manda el mensaje de solicitud del requerimiento al topic
		...
		if(count == TIME_OUT){ // Verifica si se cumplió el time out 
			if(this.respuesta.getNotadebito().isEmpty()){
				waiting = false;
				this.numberApps = 0;
		...
		waiting = false;
		this.numberApps = 0;
		if(respuesta.getNotadebito().isEmpty())
		...
		ListaNotaDebito res = respuesta;
		List<NotaDebito> notaDebito = new ArrayList<NotaDebito>();
		respuesta = new ListaNotaDebito(notaDebito);
		return res.getNotadebito(); // Retorna con la respuesta completa de todas las aplicaciones
	}
	 ...
	 */
	public void sendMessage(String idCompania) throws JMSException, NamingException{
		...
		txtMsg.setText(GET_ALL_NOTA_DEBITO_ASK + CONNECTOR + this.myQueue + CONNECTOR + idCompania);
	...
	public void onMessage(Message message)
	{
		...
		try 
		{
			...
			if(a[0].equals(GET_ALL_NOTA_DEBITO_ASK) && !a[1].equals(this.myQueue)){
				ListaNotaDebito funciones = this.master.deleteCompaniaLocal(Integer.parseInt(a[2]));
				ObjectMapper mapper = new ObjectMapper();
				String jsonString = mapper.writeValueAsString(funciones);
				System.out.println(jsonString);
				doResponseToQueue(a[1], GET_ALL_NOTA_DEBITO_REPLY + CONNECTOR + jsonString);
			}
			else if(a[0].equals(GET_ALL_NOTA_DEBITO_REPLY)){
				ObjectMapper mapper = new ObjectMapper();
				if(waiting){
					NotaDebito obj = mapper.readValue(a[1], NotaDebito.class);
					this.respuesta.getNotadebito().add(obj);
					this.numberApps++;
				}
			}
